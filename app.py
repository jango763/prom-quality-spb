...
st.markdown("""
    <style>
        ...
        /* Фикс полей ввода: делаем текст внутри инпутов ярко-белым */
...
# ==============================================================================
# 3. НАВИГАЦИОННАЯ ПАНЕЛЬ СЛУЖБЫ АПП СПБ (Сайдбар)
# ==============================================================================
with st.sidebar:
...
if user_role == "🎓 Личный кабинет Физического лица":
    ...
    with st.form("citizen_form_mono", clear_on_submit=False):
    ...
    # Блок теста компетенций с ЖЕСТКО ВЫРОВНЕННЫМИ ОТСТУПАМИ
    with st.form("test_form_mono"):
        st.markdown("<h4 style='color:#34D399; font-weight:700;'>🤖 Тест компетенций на производстве</h4>", unsafe_allow_html=True)
        st.markdown("<p style='color:#FFFFFF; font-weight:600;'>КЕЙС: Критическая аварийная ситуация: Датчик стойки управления Syntec выдал перегрев шпинделя станка ЧПУ за 20 млн рублей. Ваши действия?</p>", unsafe_allow_html=True)

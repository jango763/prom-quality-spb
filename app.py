...
# ================= ВАРИАНТ 1: КАДРОВЫЙ ХАБ (ЯНДЕКС.ПРАКТИКУМ / РАБОТА) =================
if option == "Вариант 1: Кадровый хаб (Mэтчинг Яндекса)":
    ...
    # Эмуляция базы данных студентов
    students_db = {
    ...
    with col2:
        ...
        # Алгоритм автоматического мэтчинга уровня Яндекса
...
# ================= ВАРИАНТ 2: ШЕРИНГ-ЭКОНОМИКА (АВИТО ДЛЯ БИЗНЕСА) =================
    ...
    with tab1:
        ...
        with st.form("booking_form"):
            ...
            # Расчет стоимости в реальном времени уровня Авито
            ...
            if submitted:
                if not company.strip():
                    ...
                    with sqlite3.connect('prom_quality.db') as db_conn:
                        ...
                        db_conn.commit()
                    st.success(f"Контракт сформирован! Слот в {selected_lab} забронирован на {hours} ч. Итоговая сумма: {total_cost:,} руб.")
...
# ================= ВАРИАНТ 3: СУПЕРАПП ОБРАЗОВАНИЯ (СБЕР) =================
    ...
    # Интерактивный расчет бюджетов уровня Сбера
    base_cost = 450000  # Базовая стоимость обучения в год
